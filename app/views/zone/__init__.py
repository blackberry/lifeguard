...
from app.views.zone.models import Zone, ZoneForm, ZoneTemplateForm, Cluster
...
def manage(zone_number):
  ...
  return render_template(template, form=form, zone=zone, zones=zones)
...
def edit_template(zone_number):
  ...
  return render_template('zone/template.html', form=form, zone=zone)
...
def delete(zone_number):
  ...
  if request.method == 'POST' and form.validate():
    try:
      ...
      return redirect(url_for('zone_bp.list'))
  return render_template('zone/delete.html', form=form, zone=zone)
